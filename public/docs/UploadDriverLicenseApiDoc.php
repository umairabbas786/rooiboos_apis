...
class UploadDriverLicenseApiDoc extends ApiDoc {

    function onGeneralMeta() { ?>
        <div class="api-general-meta">
            <h2 class="api-title mt-5">Upload Driver License</h2>
            <div class="api-meta">
                <p class="api-method">POST</p>
                <p class="end-point">upload_driver_license.php</p>
            </div>
        </div>
    <?php }
    ...
    function onParamsDoc() { ?>
        <table class="table table-bordered params-table">
            ...
            <tr>
                <td class="gravity-center">license_front</td>
                <td class="gravity-center">Multipart</td>
                <td class="gravity-center">True</td>
                <td> ----------- </td>
                <td>Front Image of License</td>
            </tr>
            <tr>
                <td class="gravity-center">license_back</td>
                <td class="gravity-center">Multipart</td>
                <td class="gravity-center">True</td>
                <td> ----------- </td>
                <td>Backside image of License</td>
            </tr>
        </table>
    <?php }
    ...
    function onResponseDoc() { ?>
        ...
        <p class="mt-4">Following response comes if Driver License is already Uploaded.</p>
        <pre class="response-box"><code class="language-json kill-as-bad-request <?php $this->getUniqueClass() ?>"></code></pre>
        <script>
            document.querySelector('.<?php $this->getUniqueClass(); ?>').innerHTML = JSON.stringify({
        ...
        <p class="mt-4">Following response comes if license images couldn&apos;t be saved. This happens due to server low storage or if someone messed with the server.</p>
        <pre class="response-box"><code class="language-json kill-as-bad-request <?php $this->getUniqueClass() ?>"></code></pre>
        <script>
            document.querySelector('.<?php $this->getUniqueClass(); ?>').innerHTML = JSON.stringify({
                "cab_5_response_code": 506,
                ...
                "data": {
                    "exceptions": {
                        "failed_to_save_license_images": true
                    }
                }
            }, null, '\t')
        </script>
        ...
        <p class="mt-4">Following response comes if License is uploaded successfully.</p>
        <pre class="response-box"><code class="language-json kill-as-bad-request <?php $this->getUniqueClass() ?>"></code></pre>
        <script>
            document.querySelector('.<?php $this->getUniqueClass(); ?>').innerHTML = JSON.stringify({
