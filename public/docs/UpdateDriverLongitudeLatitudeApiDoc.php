...
class UpdateDriverLongitudeLatitudeApiDoc extends ApiDoc {

    function onGeneralMeta() { ?>
        <div class="api-general-meta">
            <h2 class="api-title mt-5">Update Driver Longitude Latitude</h2>
            <div class="api-meta">
                <p class="api-method">POST</p>
                <p class="end-point">update_driver_longitude_latitude.php</p>
            </div>
        </div>
    <?php }
    ...
    function onResponseDoc() { ?>
        ...
        <p class="mt-4">Following response comes if you accidentally called wrong api, you wanted to call driver api but you called client or admin api etc.</p>
        <pre class="response-box"><code class="language-json kill-as-bad-request <?php $this->getUniqueClass() ?>"></code></pre>
        <script>
            document.querySelector('.<?php $this->getUniqueClass(); ?>').innerHTML = JSON.stringify({
                "cab_5_response_code": 410,
